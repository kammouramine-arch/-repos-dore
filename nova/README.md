...
**Nova is not another GPS. Nova is an AI driving companion.**
...
| Screen | What it does |
|---|---|
| **Splash** | Native splash hands over to an animated Nova mark while the app restores its session. |
...
The supported path is a **development build**, because Nova's native
...
code) for a quick look — every library Nova uses is bundled in Expo Go. Two
caveats: the Android map renders with Expo Go's own Google key rather than
yours, and the permission dialogs show Expo Go's wording rather than Nova's.

> **Grant location access when asked.** Nova needs a fix to centre the map, to
...
**The ports and adapters seam.** `core/domain/ports` declares what Nova needs —
...
of its step. Nova's `RouteStep` is a leg that *ends* in a maneuver, because that
...
| `react-native-svg` | The Nova mark, the maneuver arrows and the map markers — all vector, all crisp at any size. |

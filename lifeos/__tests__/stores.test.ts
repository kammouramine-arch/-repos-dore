...
describe('product catalogue', () => {
  it('maps every product id to exactly one plan and cadence', () => {
    ...
    // The six products the stores must be configured with, named from the brand so
    // a rename cannot leave the catalogue and the store consoles disagreeing.
    expect([...seen.keys()].sort()).toEqual([
      `${brand.productPrefix}.plus.monthly`,
      `${brand.productPrefix}.plus.yearly`,
      `${brand.productPrefix}.pro.monthly`,
      `${brand.productPrefix}.pro.yearly`,
      `${brand.productPrefix}.ultra.monthly`,
      `${brand.productPrefix}.ultra.yearly`,
    ]);
  });

  it('refuses a product id that is not in the catalogue', () => {
    expect(planForProduct(DEFAULT_CATALOGUE, `${brand.productPrefix}.pro.lifetime`)).toBeNull();

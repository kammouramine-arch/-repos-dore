...
describe('AI integration is real', () => {
  ...
  it('enforces the agent quota on the server, before the model is called', () => {
    const fn = fs.readFileSync(path.join(root, 'supabase/functions/ai-chat/index.ts'), 'utf8');
    // The spend happens before the Anthropic client is ever constructed.
    const spendAt = fn.indexOf('spendForOperation(admin, billing, user.id, operation)');
    const modelAt = fn.indexOf('new Anthropic(');
    expect(spendAt).toBeGreaterThan(-1);
    expect(modelAt).toBeGreaterThan(spendAt);

    // And agent runs map to the metered operation rather than a free one.
    expect(fn).toContain("agent_run: 'agent_run'");
  });

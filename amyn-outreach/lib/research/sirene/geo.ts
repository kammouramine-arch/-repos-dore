// ---------------------------------------------------------------------------
// GEOGRAPHIE FRANCAISE — departements et regions
//
// Table factuelle et stable : 101 departements, 18 regions. Sert a deriver la
// region d'un etablissement a partir de son code commune INSEE, et a decouper
// un balayage national en territoires (lot 3).
// ---------------------------------------------------------------------------

export type Departement = {
  /** Code INSEE du departement : "01" a "95", "2A", "2B", "971" a "976". */
  code: string;
  nom: string;
  region: string;
};
...
/**
 * Departement d'un code commune INSEE.
 *
 * Regle : 5 caracteres. Les DOM commencent par "97" et occupent 3 caracteres ;
 * la Corse utilise "2A" et "2B". Partout ailleurs, les deux premiers suffisent.
 */
export function departementDeCommune(codeCommune: string | null | undefined): Departement | null {
  if (!codeCommune) return null;
  const code = codeCommune.trim().toUpperCase();
  if (code.length < 2) return null;

  if (code.startsWith("97")) return PAR_CODE.get(code.slice(0, 3)) ?? null;
  return PAR_CODE.get(code.slice(0, 2)) ?? null;
}

export function departement(code: string): Departement | null {
  return PAR_CODE.get(code.trim().toUpperCase()) ?? null;
}

/** Tous les departements d'une region. */
export function departementsDeRegion(region: string): Departement[] {
  const cible = region.toLowerCase();
  return DEPARTEMENTS.filter((d) => d.region.toLowerCase() === cible);
}

export function regions(): string[] {
  return [...new Set(DEPARTEMENTS.map((d) => d.region))].sort();
}

/**
 * Resout une zone geographique ecrite en francais.
 * « 59 », « Nord », « Hauts-de-France » → la liste des departements concernes.
 */
export function resolveZone(input: string): { kind: "DEPARTEMENT" | "REGION" | "UNKNOWN"; departements: Departement[]; label: string } {
  const brut = input.trim();
  const norm = brut.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "");

  const parCode = PAR_CODE.get(brut.toUpperCase());
  if (parCode) return { kind: "DEPARTEMENT", departements: [parCode], label: parCode.nom };

  const parNom = DEPARTEMENTS.find(
    (d) => d.nom.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "") === norm,
  );
  if (parNom) return { kind: "DEPARTEMENT", departements: [parNom], label: parNom.nom };

  const region = regions().find(
    (r) => r.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "") === norm,
  );
  if (region) return { kind: "REGION", departements: departementsDeRegion(region), label: region };

  return { kind: "UNKNOWN", departements: [], label: brut };
}

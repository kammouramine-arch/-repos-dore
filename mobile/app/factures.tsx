 ...
 *
 * L'écran répond à une seule question, celle qu'un artisan se pose le matin :
 * qui me doit de l'argent ? Le restant dû passe donc avant le montant total,
 * et les factures en retard remontent en tête.
 *
...
export default function FacturesScreen() {
   ...
   */
  React.useEffect(() => {
    if (claimed.current || !devis) return;
    claimed.current = true;
    setCreating(true);
    void api.invoices
      .createFromQuote({ quoteId: devis })
  ...
  return (
    <Screen
      refreshControl={
        <RefreshControl refreshing={query.refreshing} onRefresh={() => void query.refresh({ force: true })} />
      }
    >
      {creating ? (
        <Card style={{ backgroundColor: colors.accentSoft, borderColor: colors.accentBorder }}>
          <Body style={{ color: colors.accentHover }}>
            {en ? 'Creating the invoice…' : 'Création de la facture…'}
          </Body>
        </Card>
      ) : null}

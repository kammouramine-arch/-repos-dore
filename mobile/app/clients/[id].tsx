          ...
          <View style={{ gap: spacing.md }}>
            ...
            <Card style={{ padding: 0, overflow: 'hidden' }}>
              {query.data.activity.map((event, index) => (
                <View key={event.id} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingHorizontal: spacing.lg, paddingVertical: spacing.md, borderTopWidth: index > 0 ? 0.5 : 0, borderTopColor: colors.line }}>
                  ...
                  <Ionicons name="chevron-forward" size={15} color={colors.subtle} onPress={() => router.push({ pathname: '/devis/[id]', params: { id: event.quoteId } })} />
                </View>
              ))}
            </Card>

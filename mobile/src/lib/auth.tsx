...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadSession = React.useCallback(async () => {
    ...
    if (!token) {
      setState({ status: 'deconnecte', session: null, error: null });
      return;
    }
    try {
      const session = await api.auth.me();
      setState({ status: 'connecte', session, error: null });
    } catch {
      await clearToken();
      setState({ status: 'deconnecte', session: null, error: null });
    }
  ...
  const handle = React.useCallback(async (action: () => Promise<{ token: string; session: SessionDTO }>) => {
    ...
    try {
      ...
      setState({ status: 'connecte', session: result.session, error: null });
    } catch (error) {
      const message =

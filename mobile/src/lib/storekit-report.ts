...
export function storekitReportEvents(events: Diagnostic[]) {
  return events.filter(e => e.path === 'apple-products' || e.path === 'native-storekit' || e.path === 'apple-purchase' || e.path === 'plan-change').map(e => ({
    ...
    nativeCode: e.nativeCode, cachePolicy: e.cachePolicy, path: e.path,
    storefrontId: e.storefrontId, nativeDisplayPrice: e.nativeDisplayPrice, nativeCurrency: e.nativeCurrency,
    nativePriceLocale: e.nativePriceLocale, catalogCurrency: e.catalogCurrency, catalogPriceFormatted: e.catalogPriceFormatted,
    catalogPath: e.catalogPath, catalogIntroOffer: e.catalogIntroOffer, nativeIntroOffer: e.nativeIntroOffer, expectedCurrency: e.expectedCurrency,
    currentProductId: e.currentProductId, targetProductId: e.targetProductId, priceSource: e.priceSource, transactionState: e.transactionState,
  }));
}

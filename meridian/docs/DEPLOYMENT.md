...
|---|---|---|---|
| Supabase project | `meridian-dev` | `meridian-staging` | `meridian` |
...
supabase secrets set \
  APPLE_ISSUER_ID=... \
  APPLE_KEY_ID=... \
  APPLE_BUNDLE_ID=app.meridian.planner \
  APPLE_PRIVATE_KEY="$(cat SubscriptionKey_XXXXXXX.p8)"

# Google Play
supabase secrets set \
  ANDROID_PACKAGE_NAME=app.meridian.planner \
...
1. Create the app with bundle id `app.meridian.planner`.
2. **Subscriptions** → create one group ("Meridian") with six products:

   | Product ID | Duration |
   |---|---|
   | `meridian.plus.monthly` | 1 month |
   | `meridian.plus.yearly` | 1 year |
   | `meridian.pro.monthly` | 1 month |
   | `meridian.pro.yearly` | 1 year |
   | `meridian.ultra.monthly` | 1 month |
   | `meridian.ultra.yearly` | 1 year |
...
- Privacy policy URL, terms URL (both are already linked from the paywall).
...
1. Create the app with package `app.meridian.planner`.
...
- [ ] App icons and splash replaced (`assets/`)

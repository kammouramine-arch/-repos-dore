...
- Live web signup inspection found an inconsistent legacy link flow with no code input. Website signup now explicitly requests codes. Added code confirmation, bounded requests, resend cooldown and server-authoritative continuation. Legacy links now require an explicit form submission instead of mutating verification state on GET (which email previews/scanners can visit). Mobile verification remains unchanged.
